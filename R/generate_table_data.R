...
fix_response_levels <- function(data, colname, likert_dictionary) {
  ...
  matched_idx <- which(sapply(likert_dictionary, function(scale) {
    all(response_values %in% scale)
  }))
  ...
  if (length(matched_idx) == 1) {
    ...
    levels <- data |>
      dplyr::count(!!rlang::sym(colname)) |>
      dplyr::arrange(dplyr::desc(n)) |>
      dplyr::pull(!!rlang::sym(colname))
  }
...
summarize_battery <- function(data, question_prefix, likert_dictionary) {
  out <- data |>
    ...
    dplyr::group_by(Subquestion, Response) |>
    dplyr::summarise(n = dplyr::n(), .groups = "drop") |>
    dplyr::group_by(Subquestion) |>
    dplyr::mutate(proportion = n / sum(n)) |>
    dplyr::ungroup() |>
    dplyr::filter(!is.na(Response)) |>
    dplyr::select(-Subquestion)
  out
}
...
generate_table_data <- function(input, survey_data) {
  ...
  shiny::eventReactive(input$generate_table, {
    ...
    if (stringr::str_detect(selected_question, "^Q\\d+")) {
      question_prefix <- stringr::str_extract(selected_question, "^Q\\d+")
      selected_data <- data |>
      ...
      selected_data <- data |> dplyr::select(dplyr::all_of(selected_question))
    }
    ...
    if (
      "Subquestion" %in%
        ...
        all(summarized_data$Subquestion == summarized_data$Response)
    ) {
      summarized_data <- summarized_data %>% dplyr::select(-Subquestion)
    }
