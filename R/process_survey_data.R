process_survey_data <- function(file_upload) {
  req(file_upload)  # Ensure a file is uploaded
  ...
  if (grepl("\\.csv$", file_upload$name)) {
    ...
    date <- str_extract(tmp, "\\d{4}-\\d{2}-\\d{2}")
    respondent_count <- str_remove(trimws(sub(".*[:]", "", tmp)), "\\)")
  } else {
  ...
  if (length(sentiment_columns) > 0) {
    data <- data %>% select(-matches(paste0("^(", paste(sentiment_columns, collapse = "|"), ")")))
  }
  ...
  if ("Age range" %in% colnames(data)) {
    data <- data |>  
      dplyr::mutate(
        ...
        Cohort = factor(Cohort, levels = c("18-24", "25-34", "35-44", "45-54", "55-64", "65+")),
        ...
        `Age Decile` = factor(`Age Decile`, levels = c("<20", "20-29", "30-39", "40-49", "50-59", "60+"))) |> 
      select(-yob) |> 
  ...
  if ("Home location" %in% colnames(data)) {
    data <- data |> 
      dplyr::mutate(
        `Home location` = factor(`Home location`, levels = c("West", "Midwest", "Northeast", "South"))
      )
  }
  
  if ("Combined household income" %in% colnames(data)) {
    data <- data |> 
      mutate(HHI = case_when(
        `Combined household income` %in% c("$0 - $24;999", "$25;000 - $49;999") ~ "< $50k",
        `Combined household income` %in% c("$50;000 - $74;999") ~ "$50k - $75k",
        `Combined household income` %in% c("$75;000 - $99;999") ~ "$75k - $100k",
        `Combined household income` %in% c("$100;000 - $124;999", "$150;000 - $174;999") ~ "$100k - $150k",
        `Combined household income` %in% c("$150;000 - $174;999", "$175;000 - $199;999") ~ "$150k - $200k",
        TRUE ~ "$200k+"
      ),
      HHI = factor(HHI, levels = c("< $50k", "$50k - $75k", "$75k - $100k", "$100k - $150k", "$150k - $200k", "$200k+"))) |> 
      relocate(HHI, .after = `Combined household income`)
  }
  
  if ("Ethnicity" %in% colnames(data)) {
    data <- data |> 
      mutate(Ethnicity = factor(Ethnicity, 
                                levels = c("White", "Hispanic/Latino", "Black", 
                                           "Asian", "American Indian/Alaskan Native", "Other")))
  }
  
  if ("Home geography" %in% colnames(data)) {
    data <- data |> 
      mutate(`Home geography` = factor(`Home geography`, 
